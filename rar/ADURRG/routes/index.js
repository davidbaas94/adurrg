    ...
    if(err) throw err;  
      ...
      paypal.payment.execute(paymentId, execute_payment_json, function (error, payment){
        if (error){
          ...
          db.query("INSERT INTO venta (id,  Idmanual, fecha) VALUES ('"+ req.session.userId +"', '"+ id +"', CURDATE())", function(err,VENTAcomprada){
            if (err) throw err; 
...
router.get('/info', function(req, res, next) {
  if (req.session.username){
    var id = req.query.id; 	
      ...
      db.query(sql, function (err, result){          
      if(err) throw err;                  
        db.query("SELECT * FROM manual where cantidaddispo>0 ORDER BY Idmanual DESC LIMIT 3 ", function(err,interesar){
          if (err) throw err;
...
router.get('/usuarios', function(req, res, next) {
  if (req.session.username){
    db.query("SELECT * FROM usuarios;", function(err,users){
      if (err) throw err; 
        res.render('usuarios', { title: 'Todos los usuarios', Usuarios: users, nombre: 'Hola '+ req.session.nombre });              
    });
  }
  else{         
    res.redirect('/login');  
...
router.get('/deleteuser', function(req, res, next) {
  if (req.session.username){
    db.query("DELETE FROM usuarios WHERE id = ?", req.query.id,function(err,rs){
      if(err) throw err;
...
router.get('/agregar', function(req, res, next) {
  if (req.session.username){
    db.query("SELECT * FROM galeria ORDER BY idimg DESC LIMIT 1", function(err,ultimo){
      if (err) throw err; 
      res.render('agregar', { title: 'Agregar un Nuevo Manual', Manuales:ultimo, nombre: 'Hola '+ req.session.nombre});   
    });    
  }else{         
    res.redirect('/login');         
  }  
    ...
    var sql = "INSERT INTO manual (titulo, descripcion, precio, autor, cantidaddispo,tecnologia, manual, imagen) VALUES ('"+ req.body.titulo +"', '"+ req.body.descripcion +"', '"+ req.body.precio +"','"+ req.body.autor +"','"+ req.body.cantidaddispo +"','"+ req.body.tecnologia +"','"+ manual +"','"+ req.body.imagen +"')";
      db.query(sql, function (err, result) {     
        ...
        db.query("SELECT * FROM manual", function(err,resultados){
          if(err) throw err;
          res.render('listaproduc', { title: 'Todos los Productos',Msm: 'Agregado' ,nombre: 'Hola '+ req.session.nombre , Manuales: resultados });
        });        
      });
  }else{         
    res.redirect('/login');         
...
router.get('/listaproduc', function(req, res, next) {
  if (req.session.username){
    db.query("SELECT * FROM manual", function(err,resultados){
      if(err) throw err;
      res.render('listaproduc', { title: 'Todos los Productos',Msm: '' ,nombre: 'Hola '+ req.session.nombre , Manuales: resultados });
    });
  }else{         
    res.redirect('/login');         
  } 
...
router.get('/galeria', function(req, res, next) {
  if (req.session.username){     
    res.render('galeria', { title: 'Todas las ventas', nombre: 'Hola '+ req.session.nombre , });
  }else{         
    res.redirect('/login');         
  }
});
...
router.get('/edit', function(req,res,next){
  if (req.session.username){
    db.query("SELECT * FROM manual WHERE Idmanual=?", req.query.Idmanual,function(err,resultados){
      if(err) throw err;
      res.render('edit', { title: 'Todos los Productos',nombre: 'Hola '+ req.session.nombre , Manuales: resultados });
    });
  }else{         
    res.redirect('/login');         
  } 
...
router.post('/edit/:id', function(req, res){
  if (req.session.username){    
    console.log("ESto se recupero");
    console.log(req.body);
    const { id } = req.params;
    const newCustomer = req.body;  

    db.query('UPDATE manual set ? where Idmanual = ?', [newCustomer, id],function(err,rs){
      if(err) throw err; 
        res.redirect('/listaproduc');      
    }) 
